...
class BrokenLinkCheckerBase extends Component {
	...
	addListeners() {
		ipcRenderer.on("blc-async-message-from-process", (event, response) => {
			...
			if (
				this.state.siteId !== this.props.scanSiteId &&
				![
	...
	legacyPluginDataDetected() {
		...
		if (this.state.hasOwnProperty("brokenLinks")) {
			...
			if (this.state.brokenLinks.length) {
				...
				if (
					!this.state.brokenLinks[0].hasOwnProperty("originURI") ||
					!this.state.brokenLinks[0].hasOwnProperty("dateAdded")
				) {
					return true;
				}
			}
	...
	isScanningProcessAlive = () =>
		new Promise((resolve, reject) => {
			ipcAsync("scanning-process-life-or-death")
				.then((isRunning) => {
					if (isRunning) {
						resolve(isRunning);
					} else {
	...
	updateTotalSitePosts = (prefix) =>
		new Promise((resolve, reject) => {
			...
			ipcAsync("get-total-posts", this.state.siteId, prefix)
				.then((result) => {
					this.setState((prevState) => ({
						getTotalSitePostsInProgress: false,
					}));
	...
	updateTablePrefix = () =>
		new Promise((resolve, reject) => {
			ipcAsync("get-table-prefix", this.state.siteId)
				.then((result) => {
	...
	startScan = () => {
		...
		this.testSiteRootUrlVariantsAndUpdate(siteDomain)
			.then((rootUrl) => {
				...
				if (this.state.getTotalSitePostsInProgress === false) {
					this.updateTablePrefix().then((prefix) => {
						this.updateTotalSitePosts(prefix)
							.then((totalSitePosts) => {
								// Start site tasks
								const routeChildrenProps =
									this.props.routeChildrenProps;
								const siteStatus =
									routeChildrenProps.siteStatus;

								if (
									(this.state.resultsOnScreen ||
										!this.state.brokenLinksFound) &&
									String(this.state.siteStatus) !==
										"halted" &&
									this.state.siteStatus !== null
								) {
									ipcRenderer.send(
										"analyticsV2:trackEvent",
										"v2_pro_link_checker_run_success",
										{
											linksScanned:
												this.state.numberPostsFound,
											linksBroken:
												this.state.brokenLinks.length,
										}
									);
							...
							.catch((err) =>
								console.log(
									"Error getting total site posts: " + err
								)
							);
					});
				}
			...
			.catch((err) => {
				...
				ipcRenderer.send(
					"analyticsV2:trackEvent",
					"v2_pro_link_checker_run_failure"
				);
			});
	};
	...
	cancelScan = () => {
		ipcRenderer.send(
			"analyticsV2:trackEvent",
			"v2_pro_link_checker_run_cancel"
		);
	...
	renderHeader() {
		...
		let messageLeftOfActionButtonText =
			"Last updated " + this.renderLastUpdatedTimestamp();
				...
				<div>
					<div className="LinkChecker_StartScan_Header">
						{this.state.scanInProgress &&
							this.state.numberBrokenLinksFound !== null && (
								<Title size="s">
									<span>
										Broken Links:{" "}
										<strong>
											{this.state.numberBrokenLinksFound}
										</strong>
									</span>
		...
		if (this.props.scanSiteId) {
			tooltipContent = (
				<>Scan in progress. Please wait for it to finish.</>
			);
	...
	renderProgressBarElements() {
		...
		progressCompletedPercentage =
			progressCompletedPercentage === 0 ? 1 : progressCompletedPercentage;
		...
		return (
			<a
				...
				onClick={(e) => {
					...
					ipcRenderer.send(
						"analyticsV2:trackEvent",
						"v2_pro_link_checker_open_admin_link"
					);
				}}
			>
				Fix in Admin
	...
	renderFooterMessage() {
		let message = "";
		if (this.state.firstRunComplete && !this.state.brokenLinksFound) {
		...
		if (this.state.scanInProgress && this.state.siteRootUrl === null) {
		...
		return [
			{
				key: STATUS.KEY,
				value: STATUS.TEXT,
				className: "LinkChecker_VirtualTable_Header_Status",
			},
			{
				key: ORIGIN_URL.KEY,
				value: ORIGIN_URL.TEXT,
				className: "LinkChecker_VirtualTable_Header_OriginUrl",
			},
			{
				key: LINK_URL.KEY,
				value: LINK_URL.TEXT,
				className: "LinkChecker_VirtualTable_Header_LinkUrl",
			},
			{
				key: LINK_TEXT.KEY,
				value: LINK_TEXT.TEXT,
				className: "LinkChecker_VirtualTable_Header_LinkText",
			},
			{
				key: FILL.KEY,
				value: FILL.TEXT,
				className: "LinkChecker_VirtualTable_Header_Fill",
			},
		];
	};
	...
	cellRenderer = (dataArgs) => {
		const TABLE_HEADERS = constants.TABLE_HEADERS;
		const { STATUS, ORIGIN_URL, ORIGIN_URI, LINK_URL, LINK_TEXT, FILL } =
			TABLE_HEADERS;
		...
		if (colKey === STATUS.KEY) {
			...
			return (
				<div className="LinkChecker_VirtualTable_Column_Status">
					{status}
				</div>
			);
		}
		...
		if (colKey === ORIGIN_URL.KEY) {
			...
			return (
				<div className="LinkChecker_VirtualTable_Column_OriginUrl">
		...
		if (colKey === LINK_TEXT.KEY) {
			...
			return (
				<div className="LinkChecker_VirtualTable_Column_LinkText">
					<div>{linkText}</div>
				</div>
			);
		}
		...
		if (colKey === FILL.KEY) {
			return (
				<div className="LinkChecker_VirtualTable_Column_Fill">
...
const BrokenLinkChecker = (props) => {
	...
	const updateScanSiteId = (siteId) =>
		store.dispatch(actions.updateSiteId(siteId));
